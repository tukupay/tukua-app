 ...
 * - Desk module UI: EXPO_PUBLIC_DESK_WEB_URL only — never tukua.ai (Amplify LIVE lacks /teacher/* routes)
...
/** Desk SPA UI (modules) — must be Desk Vite (:3250) or a hosted Desk build, not Amplify marketing. */
...
const ENV_PROFILES: Record<TukuaEnv, EnvProfile> = {
  ...
  production: { web: DEFAULT_WEB, deskWeb: null, nestApi: PRODUCTION_API, deskApi: PRODUCTION_API },
...
export function getDeskWebBaseUrlOrNull(): string | null {
  ...
  try {
    ...
    if (host === 'tukua.ai' || host === 'www.tukua.ai') return null;
