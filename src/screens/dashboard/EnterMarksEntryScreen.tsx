/**
 * Enter marks — Desk AssessmentsHubView parity on mobile.
 * Functionalities: pick class, edit scores, Enter→next row, Save (generate+PATCH), vertical load-more.
 */
...
export function EnterMarksEntryScreen({ route, navigation }: Props) {
  ...
  const load = useCallback(
    async (cid?: string | null) => {
      ...
      try {
        ...
        let rows = await listAssessmentMarks(examId, nextClass);
        if (rows.length === 0) {
          await generateExamMarks(examId);
          rows = await listAssessmentMarks(examId, nextClass);
        }
        setMarks(rows);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (failed.length) {
        setError(`Saved with ${failed.length} error(s). Check scores and try again.`);
      } else {
        setSaveMsg(`Saved ${ids.length} mark${ids.length === 1 ? '' : 's'}.`);
        setDirty({});
      }
      await load(classId);
    } catch (e) {
  ...
  const slice = marks.slice(0, visible);
  const classLabel =
    classOptions.find((c) => c.id === classId)?.name || 'Select class';

  return (
    <View style={styles.root}>
      <DashboardBackground />
      <View style={{ paddingTop: floatingHeaderInset(insets.top) }}>
        <ModuleBackBar onPress={() => navigation.goBack()} label="Exams" />
        <ModuleScreenHeader
          title={title || 'Enter marks'}
          description="Type scores, press Next for the following student, then Save marks."
        />
      </View>
      ...
      {loading ? (
        ...
        <ModuleEmpty title="Could not load marks" description={error} />
      ) : marks.length === 0 ? (
        <ModuleEmpty
          title="No students in this class"
          description="Pick another class or ask admin to enroll students."
        />
      ) : (
        <FlatList
          data={slice}
...
function ScrollClassChips({
  ...
  if (!options.length) {
    return <Text style={styles.muted}>No classes on your workload yet.</Text>;
  }
  return (
...
const styles = StyleSheet.create({
  ...
  muted: { color: 'rgba(255,255,255,0.55)', marginTop: 6 },
});
