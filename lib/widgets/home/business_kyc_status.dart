...
class BusinessKycStatus extends StatelessWidget {
  const BusinessKycStatus({super.key});
  ...
  IconData _icon(ProfileProvider profile) {
    ...
    if (profile.user?.profileImg == null) return HugeIcons.strokeRoundedBuilding03;
    return HugeIcons.strokeRoundedLoading03;
  }

  String? _message(ProfileProvider profile) {
    if (profile.user?.profileImg == null) return 'Complete business verification';
    if (profile.user?.kycStatus == Strings.kycPending) return 'Business KYC in progress';
    if (profile.user?.kycStatus == Strings.kycRejected) return 'Business KYC rejected - retry';
    return null;
  }

  String _actionText(ProfileProvider profile) {
    if (profile.user?.profileImg == null) return 'Verify';
  ...
  void _handleTap(BuildContext context, ProfileProvider profile) {
    if (profile.user?.profileImg == null || profile.user?.kycStatus == Strings.kycRejected) {
      ...
      showGeneralDialog(
        ...
        transitionBuilder: (context, anim1, anim2, child) => InfoAlert(anim1: anim1, isBusiness: true),
      );
    }
  }
